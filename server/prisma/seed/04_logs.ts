...
export async function seedLogs() {
  ...
  const logsMap: { [key in LogLevel]: { num: number, value: any } } = {
    [LogLevel.FATAL]: { num: 3, value: new Error('fatal error') },
    [LogLevel.ERROR]: { num: 5, value: new Error('error text') },
    [LogLevel.WARN]: { num: 1, value: new Error('warning text') },
    [LogLevel.DEBUG]: { num: 5, value: { info: 'develop info' } },
    [LogLevel.INFO]: { num: 10, value: 'some info' },
  }
